...
// -- 1 EEC histogram
// -- 1 E3C histogram
// -- 1 E3C/EEC ratio histogram (Note: dummy because not norm)
...
int main(int argc, char* argv[]) {
  ...
  TH1F *EEC = new TH1F("EEC", title + 1 + t3 + " all comb", bins, low, high);
  TH1F *EEC_SS = new TH1F("EEC_SS", title + 1 + t3 + " S+S", bins, low, high);
  TH1F *EEC_SB = new TH1F("EEC_SB", title + 1 + t3 + " S+B", bins, low, high);
  TH1F *EEC_BB = new TH1F("EEC_BB", title + 1 + t3 + " B+B", bins, low, high);

  TH1F *EECs[] = {EEC, EEC_SS, EEC_SB, EEC_BB};
  ...
  TH1F *E3C = new TH1F("E3C", title + 1 + t3 + " all comb", bins, -3, 0);
  TH1F *E3C_SS = new TH1F("E3C_SS", title + 1 + t3 + " S+S", bins, -3, 0);
  TH1F *E3C_SB = new TH1F("E3C_SB", title + 1 + t3 + " S+B", bins, -3, 0);
  TH1F *E3C_BB = new TH1F("E3C_BB", title + 1 + t3 + " B+B", bins, -3, 0);

  TH1F *E3Cs[] = {E3C, E3C_SS, E3C_SB, E3C_BB};
  ...
  for (int iEvent = 0; iEvent < events; ++iEvent) {
	...
	if (p.isCharged()) {
	  ...
	  sig_particles.push_back(fastjet::PseudoJet(p.px(), p.py(), p.pz(), p.e()));
	}//! charged particles
      }//! final particles
    }
	...
	for (int y = x+1; y < int(constit.size()); y++) {
	  ...
	  if (c.pt() > 1 && d.pt() > 1) {
	    ...
	    if (divJet) {
	      ...
	      //! Fill the general EEC histogram
	      EECs[0]->Fill(r_L, c.E()*d.E());
	      
	      //! Fill the EEC by pair type
	      if (count(sig_particles.begin(), sig_particles.end(), fastjet::PseudoJet(c.px(), c.py(), c.pz(), c.e())) > 0) {
		if (count(sig_particles.begin(), sig_particles.end(), fastjet::PseudoJet(d.px(), d.py(), d.pz(), d.e())) > 0) {
		  //S+S
		  EECs[1]->Fill(r_L, c.E()*d.E());
		} else {
		  //S+B
		  EECs[2]->Fill(r_L, c.E()*d.E());
		} 
